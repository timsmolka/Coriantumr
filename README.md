...
- **The Google-Maps-style flow:** the page finds you on opening (and remembers where you last were, so the map opens there); click a business, a house number or bare map for a **place card** (what it is, its address, how far from you, website and phone when known) with **Directions** and **Start** buttons. Directions shows the time and distance and a Start button; Start follows the route turn by turn (next-turn banner, time left, optional spoken directions, re-routing if you go off it). If the blue dot is a few houses off — a computer only has network guesses, no GPS — drag it to your real spot. Logic in `js/navigation.js`.
- **The side panel, rail and extras (like Google Maps):** a left rail (menu, Saved, Recents, Science, plus links to Logic Lab and Chernobyl) and a full-height panel with the search box on top and one view underneath — home (explore buttons, recents, saved), a place, the trip, category results, or the Science panels. Over the map: category chips (Restaurants, Coffee, Hotels, …) that list what is near where you are looking, a weather chip, a layers button (Map / Satellite / Terrain — Satellite is Esri photographs with our own white place names, faint roads and markers drawn clear over them, and the names stay clickable), and a scale bar. Right-click for "What's here?", directions from/to a spot, and copy coordinates. Directions offers the routes the server finds to choose between, with swap and change-start. Save a place with ★ and share a link to it (the page opens straight to it). On a phone the panel is a bottom sheet.
...
- **The globe:** zoom out to the whole world and the flat map turns into a globe you can spin (as in Google Maps), with your position, the destination and the route / flight arc drawn on it; zoom in and the flat map returns at the same place. It is a second MapLibre GL map (`js/globe.js`, needs MapLibre v5 and WebGL) in the same style — over satellite photographs it is a satellite globe. The "Globe view" tick box in Layers turns it off.
- **Flights and boats (estimates):** for a long trip the mode row also offers ✈️ Flight (from 300 km) and 🚢 Boat (from 100 km). There is no free source of timetables or sea lanes, so these are estimates and are labelled as such: a great-circle path drawn as dots, at jet speed (830 km/h plus 25 minutes) or ferry speed (35 km/h). A boat cannot know where land is. If no road or path joins two far-apart places (across an ocean), the flight is shown automatically. Code in `js/estimates.js`.
